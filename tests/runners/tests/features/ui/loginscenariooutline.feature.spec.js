...
test.describe('Login functionality', () => {
  ...
  test.beforeEach('Background', async ({ Given }, testInfo) => { if (testInfo.error) return;
    await Given('I navigate to the login homepage "https://practicetestautomation.com/practice-test-login"'); 
  });
  
  test.describe('Login attempts with different credentials', () => {

    test('Example #1', { tag: ['@smoke', '@regression'] }, async ({ When, Then }) => { 
      await When('I login with uname "studet1" and pwd "Password123"'); 
      await Then('I should see a "error" message "Your username is invalid!"'); 
    });

    test('Example #2', { tag: ['@smoke', '@regression'] }, async ({ When, Then }) => { 
      await When('I login with uname "student" and pwd "test123"'); 
      await Then('I should see a "error" message "Your password is invalid!"'); 
    });

    test('Example #3', { tag: ['@smoke', '@regression'] }, async ({ When, Then }) => { 
      await When('I login with uname "student" and pwd "Password123"'); 
